...
function getAdminRoutePath(moduleConfig: ModuleConfig, modelConfig: ModelConfig): string {
  ...
  if (modelConfig.parent) {
    const parentPath = modelConfig.parent.routePrefix || `${moduleConfig.plural}/${modelConfig.plural}`;
    return parentPath;
  }
  return `${moduleConfig.plural}/${modelConfig.plural}`;
}
...
const TEMPLATES: TemplateMap = {
  ...
  pageComponent: (moduleConfig: ModuleConfig, modelConfig: ModelConfig): string => {
    const className = toPascalCase(modelConfig.name);
    const routePath = getRoutePath(moduleConfig, modelConfig);
    const adminRoutePath = getAdminRoutePath(moduleConfig, modelConfig);
    const componentName = getComponentName(modelConfig);
    const pageFilePath = `src/admin/routes/${adminRoutePath}/page.tsx`;
    const typesPath = path.relative(path.dirname(pageFilePath), 'src/admin/types').replace(/\\/g, '/');
    const dataTablePath = path.relative(path.dirname(pageFilePath), 'src/admin/components/data-table-page').replace(/\\/g, '/');
    const actionMenuPath = path.relative(path.dirname(pageFilePath), 'src/admin/components/action-menu').replace(/\\/g, '/');
      ...
      return (
        <>
          <DataTablePage<${className}>
            ...
            columns={columns}
            queryKey="${toSnakeCase(modelConfig.name)}"
            dataKey="${toSnakeCase(modelConfig.name)}"
            actions={[
              {
                type: "button",
                props: {
          ...
          {editing${className} && (
            <Drawer open onOpenChange={() => setEditing${className}(null)}>
              <${className}Edit 
                ${toPascalCase(modelConfig.name)}={editing${className}} 
